...
def run_DES(params, collect=True, plot_graph=False):
    ...
    # ----------------------Input data csv------------------------
    HEAT_LOAD_DATA = PROJECT_ROOT / "data" / "inputs" / "Input_kfw55_2_el.csv"
    ...
    world.connect(hwts1[0], ctrls[0], 
              ...
              ('heat_out2.F', 'tank_connections.tank1.heat_out2_F'),('sensor_01.T', 'tank_temps.tank1.sensor_1'), ('sensor_00.T', 'tank_temps.tank1.sensor_0'),
              time_shifted=True, 
              initial_data={'heat_out.T':0, 'heat_out2.T':0,'T_mean':0, 'mass':0, 
                            'sensor_02.T':0, 'hp_out.T':0})

    world.connect(ctrls[0], hwts1[0],
        ('tank_connections.tank1.hp_out_F', 'hp_out.F'),
        ('tank_connections.tank1.heat_out_T', 'heat_out.T'),
        ('tank_connections.tank1.heat_out_F', 'heat_out.F'),
        ('tank_connections.tank1.heat_out2_F', 'heat_out2.F'),
        ('tank_connections.tank1.hp_out_T', 'hp_out.T'),
        ('hwt1_hr_1', 'hr_1.P_th_set'),
        ('tank_connections.tank1.heat_in_F', 'heat_in.F'),
        ('tank_connections.tank1.heat_in_T', 'heat_in.T'),)



    """__________________________________________Boiler_______________________________________________________________________"""

    world.connect(hwts1[0], boiler[0], ('sensor_00.T', 'temp_in'), time_shifted=True, initial_data={'sensor_00.T':0})
    world.connect(boiler[0], hwts1[0], ('temp_out', 'boiler_in.T'), ('mdot','boiler_in.F'), ('mdot_neg', 'boiler_out.F'),
...
if __name__ == "__main__":  
    ...
    input_path = PROJECT_ROOT / "data" / "inputs" / filename
