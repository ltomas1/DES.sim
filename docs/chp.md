# CHP — Configuration Guide

## Overview

The CHP model simulates a staged combined heat and power unit. It extends the gas boiler model: it selects a thermal output stage based on the current heat demand, computes outlet temperature or mass flow rate, tracks fuel consumption, and additionally outputs electrical power proportional to the thermal output at each timestep. Startup transient behaviour is also available, identical to the boiler.

## Parameters

### Required

`efficiency`, an output mode (`set_temp` or `set_flow`), a power capacity definition, and an electrical output definition are all required.
...
| Parameter | Type | Unit | Description |
|---|---|---|---|
| `heat_out_caps` | list[float] | W | Explicit list of available thermal output stages sorted in ascending order, e.g. `[0, 5000, 10000]`. The CHP picks the smallest stage that meets the current demand, or the largest stage if demand exceeds all stages. |
...
**Electrical output — Option A (nominal electrical power):**

| Parameter | Type | Unit | Description |
|---|---|---|---|
| `P_el` | float | W | Nominal electrical power output. The model internally computes the heat-to-power ratio using the nominal thermal power. |

**Electrical output — Option B (heat-to-power ratio):**

| Parameter | Type | Unit | Description |
|---|---|---|---|
| `elec_share` | float | — | Ratio of electrical to thermal output, e.g. `0.3` means the CHP produces 30% of its thermal output as electricity. Must be between 0 and 1. |

> At least one of `P_el` or `elec_share` must be provided. If both are given, `P_el` takes precedence.
...
- `P_el` — float [W]: electrical power output in the current timestep. Computed as `P_th × elec_share`.
...
- `uptime` — float [min]: time elapsed since the last ignition. Resets to 0 when the CHP turns off.

**Methods**

- `step(time)` — advances the CHP by one timestep. Selects the appropriate power stage, applies startup behaviour if configured, computes `temp_out`, `mdot`, `fuel`, and `P_el`.
...
Minimal setup using nominal electrical power, no startup transient:
